...
void Graph<NodeType, ArcType>::aStar(Node* pStart, Node* pDest, void(*pProcess)(Node*), std::vector<Node *>& path, int max, std::vector<Node *>& attemptedPath) {
	...
	//set g cost and h cost to 'infinity'
	for (int i = 0; i < max; i++) {
		nodeArray()[i]->setData(NodeType(get<0>(nodeArray()[i]->data()), 9999, 9999, get<3>(nodeArray()[i]->data()), get<4>(nodeArray()[i]->data())));
	}
	//set up priority queue
	priority_queue<Node *, vector<Node*>, NodeSearchCostComparer> pq;
	//initialise g cost of start node to 0
	pStart->setData(NodeType(get<0>(pStart->data()), 0, get<2>(pStart->data()), get<3>(pStart->data()), get<4>(pStart->data())));
	//push start node to priority queue and mark it
	pq.push(pStart);
	pStart->setMarked(true);

	//get x position and y position of destination node
	float gx = get<3>(pDest->data());
	float gy = get<4>(pDest->data());

	//calculate h cost from every node to destination node
	for (int i = 0; i < max; i++) {
	...
	//check if the size is not 0 and if the top of the priority queue is not the destination node
	while (pq.size() != 0 && pq.top() != pDest) {
		list<Arc>::const_iterator iter = pq.top()->arcList().begin();
		list<Arc>::const_iterator endIter = pq.top()->arcList().end();
		
		//iterate through nodes from the priority queue
		for (; iter != endIter; iter++) {
			//if the current node is not the previous node of the top node in the pq
			if ((*iter).node() != pq.top()->previous()) {
